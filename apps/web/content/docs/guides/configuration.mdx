---
title: Configuration
description: Customize your SecurityLayer security policy.
icon: Gear
---

SecurityLayer's configuration lives at `~/.securitylayer/`. All config files use YAML with a `version` field for schema compatibility.
...
All config files include a `version` field. When SecurityLayer loads a config file:
