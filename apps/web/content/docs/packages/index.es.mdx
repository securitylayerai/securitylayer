---
title: Paquetes
description: Arquitectura de paquetes del monorepo y cmo encaja cada paquete.
icon: Cube
---

Security Layer es un monorepo de Bun con cinco paquetes. Cada paquete tiene una responsabilidad nica y lmites bien definidos.

## Arquitectura

```
┌──────────────────────────────────────────────────────┐
│                   securitylayer (CLI)                 │
│          Comandos de usuario, setup, hooks            │
├──────────────────────────────────────────────────────┤
│                                                      │
│  ┌──────────────┐  ┌──────────────┐  ┌────────────┐ │
│  │    proxy      │  │     sdk      │  │   rules    │ │
│  │  WebSocket    │  │  En proceso  │  │   YAML     │ │
│  │  gateway      │  │  checks     │  │  rule pack │ │
│  └──────┬───────┘  └──────┬───────┘  └─────┬──────┘ │
│         │                 │                │         │
│  ┌──────┴───────┐         │                │         │
│  │   adapters    │         │                │         │
│  │  Parsing de   │         │                │         │
│  │  frames       │         │                │         │
│  └──────┬───────┘         │                │         │
│         │                 │                │         │
│  ┌──────┴─────────────────┴────────────────┴───────┐ │
│  │                    core                          │ │
│  │  Capacidades, taint, motor de reglas, pipeline,  │ │
│  │  normalizacin, escaneo de egreso, LLM judge     │ │
│  └──────────────────────────────────────────────────┘ │
└──────────────────────────────────────────────────────┘
```

## Resumen de paquetes

| Paquete | npm | Descripcin |
|---|---|---|
| [`@securitylayerai/core`](/docs/packages/core) | `@securitylayerai/core` | Motor de seguridad -- capacidades, taint, reglas, pipeline, normalizacin |
| [`@securitylayerai/rules`](/docs/packages/rules) | `@securitylayerai/rules` | Reglas base y plantillas de capacidades (YAML) |
| [`@securitylayerai/adapters`](/docs/packages/adapters) | `@securitylayerai/adapters` | Adaptadores de protocolos de agentes (OpenClaw, genrico) |
| [`@securitylayerai/proxy`](/docs/packages/proxy) | `@securitylayerai/proxy` | Proxy de seguridad WebSocket entre clientes y gateway de agentes |
| [`@securitylayerai/sdk`](/docs/sdk) | `@securitylayerai/sdk` | SDK de TypeScript para verificaciones de seguridad en proceso |
| `securitylayer` | `securitylayer` | CLI -- comandos de usuario, setup, hooks |

## Grafo de dependencias

```
securitylayer (CLI)
├── @securitylayerai/core
├── @securitylayerai/rules
└── @securitylayerai/proxy
    └── @securitylayerai/adapters

@securitylayerai/sdk
└── @securitylayerai/core

@securitylayerai/rules
└── (independiente -- solo YAML, sin dependencias de cdigo)
```

Restricciones clave:
- **core** tiene cero dependencias internas -- es la base
- **rules** es solo datos -- archivos YAML con un loader ligero, sin dependencia de core en tiempo de ejecucin
- **adapters** es independiente -- define la interfaz e implementaciones para protocolos de agentes
- **proxy** depende de adapters para el parsing de frames
- **sdk** depende de core para el pipeline de seguridad

## Desarrollo

```bash
# Instalar todas las dependencias
bun install

# Ejecutar todos los tests
bun run test

# Ejecutar tests de un paquete especfico
bun run test --filter=@securitylayerai/core

# Verificar tipos en todo el proyecto
bun run typecheck
```

<Cards>
  <Card
    title="Core"
    description="Motor de seguridad, pipeline, capacidades, seguimiento de taint."
    href="/docs/packages/core"
    icon={<Shield weight="duotone" />}
  />
  <Card
    title="Rules"
    description="Reglas base y plantillas de capacidades."
    href="/docs/packages/rules"
    icon={<Gear weight="duotone" />}
  />
  <Card
    title="Adapters"
    description="Adaptadores de protocolos de agentes para parsing de frames."
    href="/docs/packages/adapters"
    icon={<Plug weight="duotone" />}
  />
  <Card
    title="Proxy"
    description="Proxy de seguridad WebSocket para gateways de agentes."
    href="/docs/packages/proxy"
    icon={<Globe weight="duotone" />}
  />
</Cards>
