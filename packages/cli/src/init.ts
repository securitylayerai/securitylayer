...
export async function runInit(args: CliArgs): Promise<void> {
  ...
  await writeFile(join(CONFIG_DIR, "config.yaml"), toYaml(mainConfig), "utf-8");
  ...
  // Write ai-tools.yaml
  await writeFile(CLI_CONFIG_PATHS.callers, toYaml(defaultCallersConfig()), "utf-8");

  // Write projects.yaml
  await writeFile(CLI_CONFIG_PATHS.projects, toYaml(defaultProjectsConfig()), "utf-8");
